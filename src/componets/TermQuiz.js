...
const TermQuiz = ({term, list, onQuizFinish }) => {
    ...
    const postData = async () => {
        try{
            const response = await axios.post("http://127.0.0.1:8080/api/term-quiz/questions/1/answers/check", {
    ...
    const isCorrectAnswer = quizItems.id === answerId;
        ...
        {/* 용어 게임 바디 */}
        {
            quizItems.map((item) => (
                <div className={styles.quiz_content} key={item.id}>
                    ...
                    onClick={() => setUserAnswerId(item.id)}
                    >
